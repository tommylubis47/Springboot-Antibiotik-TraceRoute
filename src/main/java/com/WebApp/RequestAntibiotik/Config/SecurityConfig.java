...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception
	{
			...
			.authorizeHttpRequests()
                        .requestMatchers("/plugins/**","/dist/**","/fragments/**").permitAll()
                        .requestMatchers("/dokter/**","/dokter/fragments/**","/dokter/fragments").permitAll()
                        .requestMatchers("/user/registration/**","/user/processRegistration/**").permitAll()       
			//.requestMatchers("/").permitAll().anyRequest().authenticated()
                        .requestMatchers("/").authenticated()
                        .requestMatchers("/homepage/dokter").authenticated()
                        .requestMatchers("/user/profile").permitAll()
			.and()
			.formLogin().loginProcessingUrl("/processLogin").loginPage("/userLogin")
			.defaultSuccessUrl("/user/profile",true).permitAll(true)
			.and()
			.logout()
